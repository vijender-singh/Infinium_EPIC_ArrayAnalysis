...
#bioc_packages <- c("BiocManager", "minfi",
...
install.packages("../supportfiles/annotation/IlluminaHumanMethylationEPICmanifest.tar.gz",repos=NULL, type="source")


bioc_packages2 <- c("BiocManager", "minfi","IlluminaHumanMethylationEPICmanifest",
                   "IlluminaHumanMethylationEPICanno.ilm10b4.hg19",
                   "sva", "limma", "impute", "BiocParallel")

#' Run test
check_installed(pkgs = c(bioc_packages2, cran_packages))

#' Load all packages, if needed
lapply(c(bioc_packages2, cran_packages), require, character.only = TRUE)
...
pheno <- read.csv("mother_Pheno_QC_01.csv",as.is = T) # WRITE PHENOTYPE FILE HERE
pheno <- subset(pheno, failed == "FALSE")

#' This is the path to the folders having idats
main_dir <- "../../data/mother_idats/"  # PATH TO IDATS FOLDER HERE, this is an example path, change accordingly  
...
save(pval, beta, signalA, signalB, file = "mother_IntermediaryFiles.RData")

# Save the new phenotype file with flagged sex mismatches
write.csv(pheno, file = "mother_Pheno_QC_02.csv", row.names = F)
...
load("mother_IntermediaryFiles.RData")

beta.new <- cpg.qc(beta, signalA, signalB, pval, p.cutoff=.01,cpg.miss=.1, sample.miss=.1)

#### MOTHER SAMPLES ####
#[1] "Removed 18 samples with low signal"
#[1] "Removed 1630 CpG sites with missing data for > 0.1 of samples"
...
write.csv(beta.new, file = "mother_noob_qcd_crossReactiveProbesRemoved.csv", quote = FALSE, row.names = TRUE) # keep the noob normalized and QCed data
...
beta <- fread("mother_noob_qcd_crossReactiveProbesRemoved.csv", data.table = F) # cpgs x samples
row.names(beta)<-beta$V1
beta<-beta[,-1]

# Load the new phenotype file with QC info
pheno <- read.csv("mother_Pheno_QC_02.csv", stringsAsFactors = FALSE, header = TRUE)
row.names(pheno)<-pheno$SampleID

#Make sure that samples in pheno and beta are same.
...
print(paste0("Samples with no Race information = ", sum(is.na(phen[,race]))))
print(paste0("Samples with no Length of delievery information = ", sum(is.na(phen[,LngthDeliv]))))
print(paste0("Samples with no Head Circumference information = ", sum(is.na(phen[,HeadCircum]))))
print(paste0("Samples with no Birth Height information = ", sum(is.na(phen[,BirthHght]))))
print(paste0("Samples with no Birth Weight information = ", sum(is.na(phen[,Birthweight]))))
print(paste0("Samples with no Gestation information = ", sum(is.na(phen[,Gestation]))))
print(paste0("Samples with no Heart Rate information = ", sum(is.na(phen[,HeartRate]))))
print(paste0("Samples with no Race information = ", sum(is.na(phen[,race]))))

naindex <- (!is.na(phen[,PTSD]) & !is.na(phen[, sex]))
phen <- phen[naindex, ]
beta <- beta[, naindex]
stopifnot(all(colnames(beta) == rownames(phen)))

phen<-phen[!is.na(phen$LngthDeliv),]
beta <- beta[, which(names(beta) %in% row.names(phen))]
phen <- phen[row.names(phen) %in% names(beta), ]
beta <- beta[, order(names(beta))]
phen <- phen[order(row.names(phen)), ]
...
LngthDeliv<-as.numeric(phen[,LngthDeliv])
HeadCircum<-as.numeric(phen[,HeadCircum])
BirthHght<-as.numeric(phen[,BirthHght])
Birthweight<-as.numeric(phen[,Birthweight])
Gestation<-as.numeric(phen[,Gestation])
HeartRate<-as.numeric(phen[,HeartRate])
#sex<-as.factor(phen[,sex])
# Note we donot have "Length of delievery" data  in 2 cases.
# Lets drop them


#moddata <- model.matrix(~ptsd+age+race)
moddata <- model.matrix(~ptsd+age)

# Remaining samples
print(paste0("Remaining Samples = ", nrow(phen))) # N = 163

#[1] "Remaining Samples = 76"
...
norm <- fread("mother_noob_qcd_crossReactiveProbesRemoved.csv", data.table = F)
...
write.csv(beta,file="noob_qcd_crossReactiveProbesRemoved_combat_CP_wcovar_ptsd.csv",quote=FALSE,row.names=TRUE)
